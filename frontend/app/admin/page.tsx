        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <button className="text-primary-neon text-sm font-medium hover:underline">View All</button>
                </div>
                            ...
                            {stats?.recentOrders?.length > 0 ? (
                                stats.recentOrders.map((order: any) => (
                                    <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-900 font-medium">{order.user?.name || 'Unknown'}</td>
                                        <td className="px-6 py-4 text-sm text-gray-500">{new Date(order.createdAt).toLocaleDateString()}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900 font-bold">${order.totalAmount?.toFixed(2)}</td>
