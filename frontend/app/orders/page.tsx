    ...
    return (
        <Section variant="default" containerSize="lg">
            <SectionHeader
                ...
                title="My Orders"
                subtitle={loading ? 'Loading...' : `You have placed ${orders.length} ${orders.length === 1 ? 'order' : 'orders'}`}
            />

            {loading ? (
                <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="bg-dark-bg-tertiary rounded-2xl p-6 border border-dark-border animate-pulse h-40" />
                    ))}
                </div>
            ) : orders.length === 0 ? (
                <div className="text-center py-20 bg-dark-bg-tertiary rounded-2xl border border-dark-border">
                    ...
                    <h2 className="font-display font-bold text-2xl mb-4">No orders yet</h2>
                ...
                <div className="space-y-6">
                    {orders.map((order) => (
                        <div key={order._id} className="bg-dark-bg-tertiary rounded-2xl border border-dark-border overflow-hidden group hover:border-primary-neon/50 transition-colors">
                            <div className="p-6">
                                ...
                                <div className="space-y-4">
                                    {order.items.map((item, index) => (
                                        <div key={index} className="flex items-center justify-between py-2 border-t border-dark-border/50 first:border-0">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-medium text-dark-text-primary">{item.name}</p>
